...
use renet::{
    generate_random_bytes, DefaultChannel, RenetConnectionConfig, RenetServer, ServerAuthentication, ServerConfig,
    ServerEvent,
};
...
impl ChatServer {
        ...
        while let Some(event) = self.server.get_event() {
            match event {
                ServerEvent::ClientConnected(client_id, user_data) => {
                    ...
                    self.server
                        .send_message(client_id, DefaultChannel::Reliable, init_message);
                }
    ...
    pub fn receive_message(&mut self, client_id: u64, text: String) {
        ...
        let message = bincode::options()
            .serialize(&ServerMessages::ClientMessage(message))
            .unwrap();
        self.server.broadcast_message(DefaultChannel::Reliable, message);
    }
}
